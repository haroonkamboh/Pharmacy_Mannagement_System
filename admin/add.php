...
if ( !$connection ) {
    ...
    if ( 'addManager' == $action ) {
        ...
        if ( $name && $price && $quantity && $description  ) {
            $query = "INSERT INTO items(name,price,quantity,description,image) VALUES ('$name','$price','$quantity','$description','$image')";
            mysqli_query( $connection, $query );
            print($query);
            move_uploaded_file($_FILES["img"]["tmp_name"],$image_dir);
            header( "location:index.php?id=allItems" );
        }
    } elseif ( 'updateItem' == $action ) {
        $id = $_REQUEST['id'] ?? '';
        $name = $_REQUEST['name'] ?? '';
        $price = $_REQUEST['price'] ?? '';
        $quantity = $_REQUEST['quantity'] ?? '';
        ...
        if ( $name && $price && $quantity && $description ) {
            $query = "UPDATE items SET name='$name', price='$price', quantity='$quantity', description='$description' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allItems" );
        }
